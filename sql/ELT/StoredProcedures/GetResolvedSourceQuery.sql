CREATE PROC [ELT].[GetResolvedSourceQuery] @SourceQuery [NVARCHAR](MAX),@WaterMarkQuery [NVARCHAR](MAX),@IncrementalLoad [BIT] AS
BEGIN
 SET NOCOUNT ON;
 ...
 -- Step 6: Replace placeholder if needed
 DECLARE @FinalSourceQuery NVARCHAR(MAX);
 SET @FinalSourceQuery = 
 CASE 
 WHEN @IncrementalLoad = 1 
 THEN REPLACE(@SourceQuery, '{WaterMarkValue}', @ResolvedWatermark)
 ELSE @SourceQuery
 END;

 -- Step 7: Return the resolved query back to ADF
 SELECT @FinalSourceQuery AS ResolvedQuery;
END;
GO
