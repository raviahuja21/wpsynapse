CREATE PROC [config].[GenerateMetadataToConfigTable] @TableCatalog [NVARCHAR](255),@TableSchema [NVARCHAR](255),@TableName [NVARCHAR](255),@OverrideTableCatalog [VARCHAR](255),@TargetTableNameOverride [NVARCHAR](255),@SourceQueryWhereClause [VARCHAR](8000),@DropAndReCreateTable [bit],@OverridePIIFlag [bit] AS
begin
/*

DECLARE @TableCatalog NVARCHAR(255) = 'ice';
DECLARE @TableSchema NVARCHAR(255) = 'dbo';
DECLARE @TableName NVARCHAR(255) = 'account';
...
begin
		...
		INTO #Columns from
		config.vw_D365MetadataLookup
		WHERE TableCatalog = @TableCatalog
		 AND TableSchema = @TableSchema
		 AND TableName = @TableName
		AND (
			@OverridePIIFlag = 1 -- include ALL columns
			OR IsPIIColumn = 0 -- include ONLY non-PII when override = 0
		 );
